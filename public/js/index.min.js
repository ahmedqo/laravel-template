function capitalize(e){return"string"!=typeof e||0===e.length?e:e.charAt(0).toUpperCase()+e.slice(1)}function run(e,t){var r;let o=document.createElement("div");async function s(t){t.preventDefault(),e.loading=!0,e.rows=await n(t.target.href),e.loading=!1}async function n(r){let n=await fetch(r),_=await n.json();return!function r(n,_,i){let a="?search"+(i?"="+i:""),c=document.querySelector("#prev"),l=document.querySelector("#next");if(n){let $=t.Search+a+"&cursor="+n;if(c)c.href=$;else{let p=o.querySelector("#prev").cloneNode(!0);p.addEventListener("click",s),l?e.insertBefore(p,l):e.appendChild(p),p.title=t.Prev,p.href=$}}else c&&(c.removeEventListener("click",s),c.remove());if(_){let h=t.Search+a+"&cursor="+_;if(l)l.href=h;else{let u=o.querySelector("#next").cloneNode(!0);u.addEventListener("click",s),e.appendChild(u),u.title=t.Prev,u.href=h}}else l&&(l.removeEventListener("click",s),l.remove())}(_.prev_cursor,_.next_cursor,new URL(r).searchParams.get("search")),_.data}o.innerHTML=`
        <a id="prev" slot="end"
    ...
    `,document.querySelector("#prev")&&document.querySelector("#prev").addEventListener("click",s),document.querySelector("#next")&&document.querySelector("#next").addEventListener("click",s),!async function(){e.rows=await n(t.Search+window.location.search)}(),e.addEventListener("search",async o=>{o.preventDefault(),r&&clearTimeout(r),e.loading=!0,e.rows=await new Promise((e,s)=>{r=setTimeout(async()=>{let r=await n(t.Search+"?search="+encodeURIComponent(o.detail.data));e(r)},2e3)}),e.loading=!1})}OS.$Load(function(){document.body.removeAttribute("close");let e=document.querySelector("#trigger"),t=document.querySelector("os-sidebar");e&&t&&e.addEventListener("click",e=>{t.toggle()}),document.querySelectorAll(".nav-colors svg").forEach((e,t)=>{t<10&&(e.style.color="var(--color-"+t+")")}),document.querySelectorAll(".sys-colors svg").forEach((e,t)=>{t<10&&(e.style.color="var(--color-sys-"+t+")")})}),function(){let e=`
        :host {
            gap: 1px;
            display: flex;
            margin: 0 auto;
