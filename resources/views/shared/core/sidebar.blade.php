<neo-sidebar id="sidebar">
    ...
    <ul class="nav-colors w-full flex flex-col flex-1">
        <li class="w-full">
            <a href="{{ route('views.core.index') }}" aria-label="dashboard_page_link"
                class="w-full flex flex-wrap gap-2 px-4 py-2.5 text-start items-center outline-none lg:rounded-e-full hover:text-x-white hover:bg-x-acent focus:text-x-white focus:bg-x-acent focus-within:text-x-white focus-within:bg-x-acent {{ Core::matchRoute('dashboard') ? 'bg-x-prime text-x-white' : 'text-x-black' }}">
        ...
        <li class="w-full">
            <a href="{{ route('views.users.index') }}"
                class="w-full flex flex-wrap gap-2 px-4 py-2.5 text-start items-center outline-none lg:rounded-e-full hover:text-x-white hover:bg-x-acent focus:text-x-white focus:bg-x-acent focus-within:text-x-white focus-within:bg-x-acent {{ Core::matchRoute('users') ? 'bg-x-prime text-x-white' : 'text-x-black' }}">
                <svg class="block w-5 h-5 pointer-events-none" fill="currentcolor" viewBox="0 -960 960 960">
                    <path
